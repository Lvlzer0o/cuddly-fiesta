...
class PWaveGenerator(ECGSegmentGenerator):
    ...
    def __init__(
        ...
        self.target_duration_ms = (
            target_duration_ms  # Default: 80ms (2 small squares)
        )
        self.target_amplitude_mv = (
            target_amplitude_mv  # Default: 0.2mV (2 small squares)
        )
    ...
    def validate_segment(self, segment_name, time, voltage):
        ...
        timing_valid, _ = self.validator.validate_timing(
            segment_name, (max(time) - min(time)) * 1000
        )
        amplitude_valid, _ = self.validator.validate_amplitude(
            segment_name, max(voltage) - min(voltage)
        )
    ...
    def _validate_parameters(self):
        ...
        self.logger.info("P-Wave Parameter Validation:")
    ...
    def generate_p_wave(self, start_time_ms=0):
        ...
        snapped_duration_ms = self.validator.snap_to_grid_time(
            self.target_duration_ms
        )
        snapped_amplitude_mv = self.validator.snap_to_grid_voltage(
            self.target_amplitude_mv
        )
        ...
        voltage_array = self._generate_p_wave_morphology(
            time_array,
            snapped_start_ms / 1000,
            snapped_duration_ms / 1000,
            snapped_amplitude_mv,
        )
    ...
    def _generate_p_wave_morphology(
        ...
        if current_max > 0:
            ...
            self.logger.warning(
                "P-wave morphology generation resulted in zero amplitude"
            )
    ...
    def generate_with_baseline(
        self, baseline_duration_sec=5, p_wave_start_ms=1000
    ):
    ...
    def plot_with_baseline(
        self, save_filename="p_wave_with_baseline.png", output_dir=None
    ):
        ...
        ax.set_title(
            "P-Wave on Calibrated ECG Baseline", fontsize=14, fontweight="bold"
        )
...
def main():
    ...
    p_wave_data, validation_passed = p_gen.test_isolation(
    ...
    if validation_passed:
        ...
        p_gen.plot_with_baseline(
            "p_wave_clinical_demo.png", output_dir=output_dir
        )
