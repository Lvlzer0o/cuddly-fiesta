    ...
    def __init__(self):
        ...
        self.timing_constraints = {
            ...
            "ST_segment": {
                ...
                "max": 120,
                "target": 100,
            },  # Ventricular repolarization plateau
            "T_wave": {
                "min": 120,
                "max": 160,
                "target": 140,
...
class ECGSegmentGenerator:
    ...
    def __init__(self, sampling_rate=1000, enable_noise=False):
        ...
        self.baseline_generator = ECGBaseline(
            duration_sec=10, sampling_rate=sampling_rate
        )
        self.grid_specs = self.baseline_generator.get_baseline_data()[
            "grid_specs"
        ]
    ...
    def plot_segment_isolation(
        self, time, signal, segment_name, save_filename=None, output_dir=None
    ):
        ...
        temp_baseline = ECGBaseline(
            duration_sec=max(time), sampling_rate=self.sampling_rate
        )
        ...
        # Plot segment
        ax.plot(
            time, signal, "b-", linewidth=2.5, label=f"{segment_name} Segment"
        )
        ...
        ax.set_title(
            f"{segment_name} - Isolation Test (Grid Aligned)",
            fontsize=14,
            fontweight="bold",
        )
        ax.legend()
        ...
        ax.text(
            ...
            bbox=dict(
                boxstyle="round,pad=0.4", facecolor="lightgreen", alpha=0.8
            ),
        )
    ...
    def validate_segment(self, segment_name, time, signal):
        ...
        duration_tolerance = max(
            0.1, FLOATING_POINT_TOLERANCE * abs(snapped_duration)
        )
        amplitude_tolerance = max(
            0.001, FLOATING_POINT_TOLERANCE * abs(snapped_amplitude)
        )

        grid_aligned = (
            abs(duration_ms - snapped_duration) < duration_tolerance
            and abs(amplitude_mv - snapped_amplitude) < amplitude_tolerance
        )
        ...
        if grid_aligned:
            ...
            print(
                f"❌ Grid alignment: Duration off by {abs(duration_ms - snapped_duration):.1f}ms, "
