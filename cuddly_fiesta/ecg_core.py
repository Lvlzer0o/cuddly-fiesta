...
class ECGCore:
    ...
    def validate_grid_integrity(self) -> bool:
        ...
        grid_time_resolution = (
            self.grid.SMALL_SQUARE_TIME_SEC / 10
        )  # 10 samples per small square min
...
class ArrhythmiaPattern(ABC):
    ...
    def __init__(
        self, name: str, lead_modifiers: Optional[Dict[str, Dict]] = None
    ):
        self.name = name
        self.segments = []
...
def main():
    ...
    print(
        f"   Grid scaling: {ecg.grid.PAPER_SPEED_MM_PER_SEC} mm/sec, {ecg.grid.VOLTAGE_SCALE_MM_PER_MV} mm/mV"
    )
