...
class ECGBaseline:
    ...
    def _add_ecg_markers(self, ax):
        ...
        ax.text(time_marker_x - 0.5, time_marker_y - 0.15, 
        ...
        ax.text(voltage_marker_x + 0.15, voltage_marker_y - 0.5, 
...
class ClinicalValidator:
    ...
    def validate_timing(self, segment_name, duration_ms):
        ...
        if segment_name not in self.timing_constraints:
            return True, f"⚠️ No timing constraints defined for segment: {segment_name}"
    ...
    def validate_amplitude(self, segment_name, amplitude_mv):
        ...
        if segment_name not in self.amplitude_constraints:
            return True, f"⚠️ No amplitude constraints defined for segment: {segment_name}"
...
class AtrialFibrillation(ArrhythmiaPattern):
    ...
    def __init__(self, heart_rate_bpm: int = 90, duration_sec: float = 3.0):
        ...
        if not (60 <= heart_rate_bpm <= 150):
            raise ValueError(f"Heart rate {heart_rate_bpm} outside range (60-150 bpm)")
...
class VentricularTachycardia(ArrhythmiaPattern):
    ...
    def __init__(self, heart_rate_bpm: int = 160, duration_sec: float = 3.0):
        ...
        if not (120 <= heart_rate_bpm <= 250):
            raise ValueError(f"Heart rate {heart_rate_bpm} outside range (120-250 bpm)")
...
def demo_baseline():
    ...
    plt.savefig(output_dir / 'ecg_baseline_demo.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def demo_single_beat():
    ...
    ecg.add_waveform_segment(t_wave, start_time_sec=0.88)  # ST segment of 120ms
    ...
    plt.savefig(output_dir / 'single_heartbeat_demo.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    print("✅ Single heartbeat demo saved as 'output/single_heartbeat_demo.png'")
...
def demo_arrhythmias():
    ...
    patterns = [
        ...
        ("Ventricular Tachycardia", VentricularTachycardia(heart_rate_bpm=160))
    ]
...
def run_animation(multi_lead=False, interval_ms=40):
    ...
    for i in range(5):
        ...
        for segment_def in pattern_copy.define_pattern():
            segment = segment_def['segment']
            start_time = segment_def['start_time_sec'] + i * pattern.rr_interval_sec
...
Examples:
  ...
  %(prog)s --gui                # Run interactive GUI (requires tkinter)
    ...
    parser.add_argument(
        '--animate',
        action='store_true',
        help='Run real-time ECG animation'
    )
    ...
    parser.add_argument(
        '--gui',
        action='store_true',
        help='Run interactive GUI application'
    )
